...
public class GameBoard {
	...
	private String title;
 	...
 	/*
 	 * Print out selected question
 	 * Print out choices
 	 * Use scanner to ask for input
 	 * 
 	 * ====================================================================
 	 * Ex:
 	 * [Question]
 	 ...
 	 * 2 [Choice at index 2]
 	 * 3 [Choice at index 3]
 	 * Enter the number corresponding to your question: [player response]
 	 * ====================================================================
 	 * 
 	 * If player response doesn't work
 	 * say: Invalid answer, please select the question using the corresponding index: [player response]
 	 * 
 	 * If player answers correctly, return and remove value from question
 	 * If player answers incorrectly, remove value from question and return 0
 	 * 
 	...
 	public boolean hasBeenAsked(int catIndex, int qIndex) {
 		if(cat[catIndex].getQuestion(qIndex).getValue() == 0) return true;
 		return false;
 	}
	...
	public String toString() {
		...
		/*  Create colWidth variable and set it to largest category name
		 *  use "-" and "+" to create borders
		 *  
		 *  row separators should begin and end with +
		 *  should contain (colWidth + 1) * size - 1
		 *  
		 *  Game board title will be at the top of the trivia game enclosed in a large box
		 *  
		 *  Use "|" to separate columns
		 *  
		 *  when creating rows print row then row separator
		 *  for loop will begin once categories are written
		 *  
		 *  for this comment, stringSize will refer to whatever is being printed in a cell
		 *  there should be colWidth - stringSize # of spaces in each cell
		 *  If there are an odd number of spaces, #ofspaces/2 will be at start Math.ceil(#ofspaces/2) will be at end
		 *  
	...
	public Category getCategory(int index) { return cat[index]; }
